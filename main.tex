...
Does it approach a limit cycle? This is more likely but we'll need to prove this somehow. We can use the Poincaré-Bendixson theorem to help us analyze long-term behavior here. 
...
of the vector field is negative which would imply that the system is dissipative and trajectories are confined to some
...
However, this still isn't enough yet. We can also analyze the nullclines to gain insight into the behavior of trajectoris.
We see that to leading order, the cubic nullcline (1) is dominated by the $-v^3/3$ term for large $|v|$, which will 
cause trajectories to be pushed back towards the origin when $|v|$ is large.
\begin{enumerate}
...
\end{enumerate}
So this loosely suggests that we have a trapping region in phase space and since at $I> I_H$ the stationary point is
unstable, we can conclude that there exists a limit cycle for $I> I_H$ which corresponds to periodic spiking behavior
in the neuron.
To test our hypothesis, we can run various simulations and study the plots generated. Firstly, we can plot the phase portraits of the system for each value of $I$. The phase portraits show the trajectories of the system in the $v$-$w$ plane, along with the nullclines and stationary points. 
\begin{figure} [H]
    \centering
